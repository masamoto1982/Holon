...
const initializeBuiltins = () => {
    ...
    dictionaryOps.define("I", new Combinator(
        directApply(x => x),
        { name: "I", description: "Identity: I x = x", isBuiltin: true }
    ));
        ...
        (operands) => {
            ...
            if (operands.length === 1) {
                ...
                return new Combinator(
                    directApply(_ => x),
                    { name: "K-partial", description: "K x - returns x regardless of argument" }
                );
            }
            ...
            return new Combinator(
                ...
                { name: "K", description: "Constant function constructor", isBuiltin: true }
            );
        },
        { name: "K", description: "Constant: K x y = x", isBuiltin: true }
    ));
        ...
        (operands) => {
            ...
            if (operands.length === 2) {
                ...
                return new Combinator(
                    ...
                    { name: "S-partial-xy", description: "S x y - waits for z" }
                );
            }
            ...
            if (operands.length === 1) {
                ...
                return new Combinator(
                    ...
                    { name: "S-partial-x", description: "S x - waits for y and z" }
                );
            }
            ...
            return new Combinator(
                ...
                { name: "S", description: "Combinator S", isBuiltin: true }
            );
        },
        { name: "S", description: "Substitution: S x y z = x z (y z)", isBuiltin: true }
    ));
            ...
            if (operands.length >= 2) {
                ...
                if (a instanceof Fraction) {
                    return new Combinator(
                        ...
                        { name: "ADD-partial", description: "Partially applied ADD" }
                    );
                } else {
                ...
                return new Combinator(
                    ...
                    { name: "ADD", description: "Addition operator", isBuiltin: true }
                );
            }
        },
        { name: "ADD", description: "Adds two numbers", isBuiltin: true }
    ));
            ...
            if (operands.length >= 2) {
                ...
                if (a instanceof Fraction) {
                    return new Combinator(
                        ...
                        { name: "SUB-partial", description: "Partially applied SUB" }
                    );
                } else {
                ...
                return new Combinator(
                    ...
                    { name: "SUB", description: "Subtraction operator", isBuiltin: true }
                );
            }
        },
        { name: "SUB", description: "Subtracts second number from first", isBuiltin: true }
    ));
            ...
            if (operands.length >= 2) {
                ...
                if (a instanceof Fraction) {
                    return new Combinator(
                        ...
                        { name: "MUL-partial", description: "Partially applied MUL" }
                    );
                } else {
                ...
                return new Combinator(
                    ...
                    { name: "MUL", description: "Multiplication operator", isBuiltin: true }
                );
            }
        },
        { name: "MUL", description: "Multiplies two numbers", isBuiltin: true }
    ));
            ...
            if (operands.length >= 2) {
                ...
                if (a instanceof Fraction) {
                    return new Combinator(
                        ...
                        { name: "DIV-partial", description: "Partially applied DIV" }
                    );
                } else {
                ...
                return new Combinator(
                    ...
                    { name: "DIV", description: "Division operator", isBuiltin: true }
                );
            }
        },
        { name: "DIV", description: "Divides first number by second", isBuiltin: true }
    ));
    
    // 比較演算
    dictionaryOps.define("EQ", new Combinator(
        (operands) => {
            if (operands.length < 2) throw new Error("EQ requires two arguments");
            const a = operands[0];
            const b = operands[1];
            ...
            return new Fraction(a.equals(b) ? 1 : 0, 1);
        },
        { name: "EQ", description: "Tests if values are equal", isBuiltin: true }
    ));
    
    dictionaryOps.define("LT", new Combinator(
        (operands) => {
            if (operands.length < 2) throw new Error("LT requires two arguments");
            const a = operands[0];
            const b = operands[1];
            ...
            return new Fraction(a.lessThan(b) ? 1 : 0, 1);
        },
        { name: "LT", description: "Tests if first value is less than second", isBuiltin: true }
    ));
    
    dictionaryOps.define("GT", new Combinator(
        (operands) => {
            if (operands.length < 2) throw new Error("GT requires two arguments");
            const a = operands[0];
            const b = operands[1];
            ...
            return new Fraction(a.greaterThan(b) ? 1 : 0, 1);
        },
        { name: "GT", description: "Tests if first value is greater than second", isBuiltin: true }
    ));
    ...
    dictionaryOps.define("PRINT", new Combinator(
        ...
        { name: "PRINT", description: "Prints a value", isBuiltin: true }
    ));
    ...
    dictionaryOps.define("PRINTLN", new Combinator(
        ...
        { name: "PRINTLN", description: "Prints a value followed by a newline", isBuiltin: true }
    ));
    ...
    dictionaryOps.define("TRUE", new Combinator(
        ...
        { name: "TRUE", description: "TRUE x y = x", isBuiltin: true }
    ));
    ...
    dictionaryOps.define("FALSE", new Combinator(
        ...
        { name: "FALSE", description: "FALSE x y = y", isBuiltin: true }
    ));
    ...
    dictionaryOps.define("IF", new Combinator(
        ...
        { name: "IF", description: "Conditional: IF cond then else", isBuiltin: true }
    ));
    ...
    dictionaryOps.define("Y", new Combinator(
        (operands) => {
            ...
            const recursion = new Combinator(
                ...
                { name: "Y-recursion", description: "Recursion helper" }
            );
            
            return recursion;
        },
        { name: "Y", description: "Y-combinator for recursion: Y f = f (Y f)", isBuiltin: true }
    ));
};
...
const parseExpression = (tokens, startIndex) => {
   ...
   // オペレータを解析
   const { expr: operator, endIndex: operatorEndIndex } = parseExpression(tokens, i);
   ...
   while (i < tokens.length && tokens[i] !== ')') {
       const { expr: operand, endIndex: operandEndIndex } = parseExpression(tokens, i);
...
const parseDefinition = (tokens) => {
   ...
   if (expressionTokens[0] === '(') {
       const { expr } = parseExpression(expressionTokens, 0);
