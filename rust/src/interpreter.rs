...
impl Interpreter {
  ...
  fn execute_tokens_with_context(
    &mut self,
    tokens: &[Token],
    in_vector: bool
  ) -> Result<(), String> {
            ...
            match name.as_str() {
              ...
              "BEGIN" => {
                ...
                match end_type.as_str() {
                  ...
                  "WHILE" => {
                    let (while_tokens, repeat_consumed) = self.collect_while_repeat(
                      &tokens[i + 1 + consumed..]
                    )?;
                  ...
                  _ => {
                    return Err(format!("Unknown loop end type: {}", end_type));
                  }
                }
                i += consumed;
              }
  ...
  fn execute_builtin_with_comment(
    &mut self,
    name: &str,
  ...
  fn op_begin_until(&mut self, loop_body: Vec<Token>) -> Result<(), String> {
    ...
    loop {
      ...
      match condition.val_type {
        ...
        _ => {
          return Err("UNTIL requires a boolean".to_string());
        }
      }
  ...
  fn op_begin_while_repeat(
    &mut self,
    condition_body: Vec<Token>,
    loop_body: Vec<Token>
  ) -> Result<(), String> {
    let mut count = 0;
    const MAX_ITERATIONS: i32 = 100000;
    ...
    loop {
      ...
      match condition.val_type {
        ...
        _ => {
          return Err("WHILE requires a boolean".to_string());
        }
      }
  ...
  pub fn delete_word(&mut self, name: &str) -> Result<(), String> {
    ...
    if let Some(dependents) = self.dependencies.get(name) {
      if !dependents.is_empty() {
        ...
        return Err(
          format!("Cannot delete '{}' because it is used by: {}", name, dependent_list.join(", "))
        );
      }
    }
  ...
  fn op_drop(&mut self) -> Result<(), String> {
    if self.stack.pop().is_none() { Err("Stack underflow".to_string()) } else { Ok(()) }
  }
  ...
  fn op_del(&mut self) -> Result<(), String> {
    if let Some(val) = self.stack.pop() {
      match val.val_type {
        ValueType::String(name) => { self.delete_word(&name.to_uppercase()) }
  ...
  pub fn get_custom_words_info(&self) -> Vec<(String, Option<String>, bool)> {
    let mut words: Vec<(String, Option<String>, bool)> = self.dictionary
      ...
      .filter(|(_, def)| !def.is_builtin)
      .map(|(name, def)| {
        let is_protected = self.dependencies
          .get(name)
